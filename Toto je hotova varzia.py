...
DB_PATH = '/home22/server/telegram_bot/glass_calculator.db'
...
class FavoriteGlass(Base):
    __tablename__ = 'favorite_glasses'
...
class GlassCuttingBot:
    ...
    def setup_handlers(self):
        ...
        def handle_custom_dimensions(message):
            try:
                ...
                logging.error(f"Chyba pri spracovaní vlastných rozmerov: {str(e)}")
        ...
        def handle_glass_selection(call):
            try:
                ...
                if not glass:
                    logging.error(f"Sklo s ID {glass_id} nebolo nájden")
        ...
        @self.bot.message_handler(commands=['favorite'])
        def add_favorite(message):
            try:
                glass_id = int(message.text.replace('/favorite', '').strip())
                
                session = Session()
                favorite = FavoriteGlass(user_id=message.chat.id, glass_id=glass_id)
                session.add(favorite)
                session.commit()
                
                self.bot.reply_to(message, "✅ Sklo bolo pridané medzi obľúbené")
                
            except Exception as e:
                logging.error(f"Chyba pri pridávaní obľúbeného: {str(e)}")
                self.bot.reply_to(message, "❌ Nastala chyba pri pridávaní obľúbeného skla")
            finally:
                session.close()
    ...
    def generate_pdf(self, chat_id: int, glass_id: int) -> io.BytesIO:
        ...
        try:
            ...
            c.drawString(50, y, f"Celková cena: {total_price:.2f}€")
            
            # Dátum a čas
            c.setFont("Helvetica", 10)
            y -= 20
            c.drawString(50, y, f"Dátum vytvorenia: {datetime.now().strftime('%d.%m.%Y %H:%M')}")
    ...
    try:
        ...
        if not session.query(GlassCategory).first():
            ...
            glasses = [
                ...
                Glass(name="2001 oranový klasik", category=categories["LACOBEL"], price_per_m2=14.88, cutting_fee=5.0, min_area=0.1),
